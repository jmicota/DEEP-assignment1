...
LABELS_INFO             = ['company', 'year', 'day', 'quarter', 'expert1_prediction',
                            'expert2_prediction', 'sentiment_analysis', 'm1', 'm2', 'm3',
                            'm4']
KEYS_1 = 'company'
...
def process_values(data):
    ...
    for n, group in data:
        print("N: ", n)
        ### should it be list(v[1:]) to include days and quarters??
        X.append([list(v[3:]) for v in group.values])
    return X
...
def process_values_and_derive_labels(data):
    ...
    X = process_values(data)
    y = list()
    
    # For each day
    for _, group in data:
        # Remove the stock price from the input data
        stock_improvement = 0
...
def process_training_data(info_company, info_quarter, info_daily, info_prices,
                          batch_size, split=80):
    ...
    input_data = pd.merge(left=df_info, right=df_market_segments, 
                          left_on=KEYS_1, right_on=KEYS_1)
    input_data = pd.merge(left=input_data, right=df_market_analysis,
                          left_on=KEYS_2, right_on=KEYS_2)
    input_data = pd.merge(left=input_data, right=df_stock_prices,
                          left_on=KEYS_3, right_on=KEYS_3)
...
def prediction_dataloaders(info_company, info_quarter, info_daily):
    ...
    # TODO: PICK UP HERE (smth messed up with preprocessing, testing batch (passed to predict) has
    # size 10 instead of 11 ???????
    print(input_data[:10])
    input_data = standardize_data(input_data)
    print(input_data)
